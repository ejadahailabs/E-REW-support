...
|---|---|---|
| `E-REW@90a5f50` (MAIN 6899c4d + branch `work/620-surface-links` — **RC6 preview**, cut before the branch's CI finished, at the owner's request; everything in the previous slot, plus: the links on a requirement card work again — a file path with underscores in it (`component_framework/src/app_manager.cpp`) was being read as Markdown emphasis, which both mangled the text you see on the Implements and Verifies lanes and corrupted the target the click posted, so clicking opened nothing and said nothing; paths are now printed as code spans and the card's link targets never reach the Markdown parser, in the form and in hovers #620. Full local gate suite (`npm run check`) green on this tree; branch CI still running) | 2026-09-02 | `47f3b80aff5008480962e6b951676d05dd1710c516274ef920a245e29485caf7` |
